  ...
  <div v-loading.fullscreen.lock="loading">
    ...
    >
      <div class="generateOrder">
        ...
        <div class="generateOrder_one mt_20">采购信息</div>
        <div class="generateOrder_three mt_20">
          <span>采购单号：</span>
          {{ purchaseNo }}
          <a class="ml_20" type="primary" @click="clickCopy(purchaseNo)"
            >点击复制</a
          >
        </div>
        <div
          v-for="(
            item, index
          ) in buyerTranManagerAddOrderObject.supplierInfoList"
          :key="index"
        >
          ...
          <div>
            <cu-table
              ...
              :tableData="item.orderJcGoodsList"
            >
        ...
        <div class="generateOrder_five mt_20">
          ...
          <div class="generateOrder_five_right">
            <div>采购总金额：{{ AllgenerateOrderData.amount }}元</div>
            <div class="mt_10">
              运费合计：
              {{ AllgenerateOrderData.transportFeeS }}
              元
            </div>
            <div class="mt_10">
              应付总金额：
              <span style="color: #ff480e">{{
                AllgenerateOrderData.payAmount
              }}</span>
              元
            </div>
          </div>
      ...
      <div class="onlinePayment">
        <div class="onlinePayment_one mt_40">
          ...
          <div class="flex" style="color: #999999">
            支付金额：
            <span style="color: #ff480e"
              >￥{{ AllgenerateOrderData.payAmount }}</span
            >&nbsp;元
          </div>
        </div>
        <el-radio-group v-model="payType" style="width: 100%">
          <div class="onlinePayment_two flex_ac pl_20 mt_20">
            <el-radio
              :label="1"
              :disabled="payBalance < AllgenerateOrderData.payAmount"
            >
              <span class="el_radio_1"> 账号余额支付 </span>
              <span class="el_radio_1"> 可用余额(元)：{{ payBalance }} </span>
              <span
                class="el_radio_1"
                v-if="payBalance < AllgenerateOrderData.payAmount"
              >
                余额不足
              </span>
            </el-radio>
          ...
          <div class="onlinePayment_two flex_ac pl_20 mt_20">
            <el-radio
              :label="2"
              :disabled="payCredit < AllgenerateOrderData.payAmount"
            >
              <span class="el_radio_1"> 信用付款 </span>
              <span class="el_radio_1"> 可用额度(元)：{{ payCredit }} </span>
              <span
                class="el_radio_1"
                v-if="payCredit < AllgenerateOrderData.payAmount"
              >
                余额不足
              </span>
            </el-radio>
...
export default {
  data() {
    return {
      ...
      AllgenerateOrderData: [], //一键采购接口回参数据
      generateOrderDataTitleNameList: [], //一键采购接口回参数据数据名列表 ['amount', 'goodsCount', 'payAmount', 'transportFeeS', 'supplier2', 'supplier1', 'supplierOrder1', 'supplierOrder2', 'goodsCategoryCount']
      ...
      buyerTranManagerAddOrderObject: {
        ...
        fkType: 2, //支付方式 1在线支付 2货到付款
        bussType: 1, // 业务类型 1 采购单  2 购物车,
  ...
  mounted() {
    this.buyerTranManagerAddOrderObject.fkType = 2; //支付方式 1在线支付，2货到付款
    this.purchaseNo = this.listItem.purchaseNo; //采购单号
    ...
    this.purchaseOnekeyBuyerApi(); // 一键采购-接口
  },
  methods: {
    // 获取采购商默认地址-接口
    ...
    // 一键采购-接口
    purchaseOnekeyBuyerApi() {
      this.loading = true;
      let valueData = new FormData();
      valueData.append("goodsType", this.listItem.goodsType);
      valueData.append("purchaseId", this.listItem.id);
      purchaseOnekeyBuyer(valueData)
        .then((res) => {
          this.loading = false;
          if (res.code == 200) {
    ...
    purchaseOnekeyBuyerDataProcessing() {
      //一键采购子集数据名列表 ['amount', 'goodsCount', 'payAmount', 'transportFeeS', 'supplier2', 'supplier1', 'supplierOrder1', 'supplierOrder2', 'goodsCategoryCount']
      this.generateOrderDataTitleNameList = Object.keys(
        this.AllgenerateOrderData
      );
      let supplierOrderList = []; //子层级标题头
      let supplier = []; //子层级表内容
      let arr = JSON.parse(JSON.stringify(this.generateOrderDataTitleNameList));
      this.generateOrderDataTitleNameList.forEach((element) => {
        if (element.indexOf("supplierOrder") != -1) {
          supplierOrderList.push(element); //子层级标题头
        }
        arr.forEach((elemen, index) => {
          if (
            elemen.indexOf("supplierOrder") != -1 ||
            elemen.indexOf("amount") != -1 ||
            elemen.indexOf("goodsCategoryCount") != -1 ||
            elemen.indexOf("goodsCount") != -1 ||
            elemen.indexOf("payAmount") != -1 ||
            elemen.indexOf("transportFeeS") != -1
          ) {
            arr.splice(index, 1);
          }
        });
      ...
      this.buyerTranManagerAddOrderObject.supplierInfoList = []; //置空后重新赋值
      supplierOrderList.forEach((name) => {
        let objRequest = {
          goodsCategoryCount:
            this.AllgenerateOrderData[name].goodsCategoryCount, //商品种类
          goodsCount: this.AllgenerateOrderData[name].goodsCount, //商品数量
          supplierId: this.AllgenerateOrderData[name].supplierId, //供货商ID
          supplierName: this.AllgenerateOrderData[name].supplierName, //供货商姓名
          transportFee: this.AllgenerateOrderData[name].transportFee, //运费
          amount: this.AllgenerateOrderData[name].subtotal, //金额
          orderType: this.listItem.goodsType, //判断订单类型
          //商品明细列表
          orderJcGoodsList: [],
        };
      ...
      supplier.forEach((name) => {
        this.AllgenerateOrderData[name].forEach((element) => {
          this.buyerTranManagerAddOrderObject.supplierInfoList.forEach(
            (elemen) => {
              if (element.supplierId == elemen.supplierId) {
                element.price = element.buyPrice;
                elemen.orderJcGoodsList.push(element);
              }
            }
          );
        });
    ...
    buyerTranManagerPaymentApi() {
      this.loading = true;
      let valueData = {
        amount: this.AllgenerateOrderData.payAmount,
