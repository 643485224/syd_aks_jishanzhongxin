<template>
  <!--中心商城页面->美食点击某个商品->点击立即购买按钮->生成订单弹框 -->
        ...
        <div class="generateOrder_two mt_20">
          ...
          <span class="mr_40"
            >收货人联系电话：{{
              ...
              receivingAddressInformation.phone
            }}</span
          >
          <span class="mr_40"
            >收货地址：{{
              receivingAddressInformation.consigneeArea ||
              receivingAddressInformation.areacode
            }}</span
          >
          <span class="mr_40"
            >详细地址：{{
              receivingAddressInformation.consigneeAddress ||
              receivingAddressInformation.address
            }}</span
          >
          <span v-if="receivingAddressInformation.zipCode" class="mr_40"
        ...
        <div v-for="(item, index) in AllgenerateOrderData" :key="index">
          <div class="generateOrder_four mt_20">
            <div class="generateOrder_bodytext">
              <!-- <span class="mr_40">订单{{ index + 1 }}：{{ item.supplierName }}</span> -->
              <!-- <span class="mr_40">种类(个)：{{ item.goodsCategoryCount }}</span> -->
              <span class="mr_40"
                >数量({{ orderParams.type }})：{{ orderParams.count }}</span
              >
              <span class="mr_40"
                >小计金额(元)：{{ orderTotalSum.amount }}</span
              >
          ...
          <div>
            <cu-table
              ...
              :tableData="item.centerShopNowVo"
            >
...
export default {
  ...
  mounted() {
    this.buyerTranManagerAddOrderObject.bussId = this.orderParams.id; //// 业务id  采购单id/购物车id,
  ...
  methods: {
          ...
          if (res.code == 200) {
            if (res.data) {
              ...
              this.buyerTranManagerAddOrderObject.consigneeArea =
                this.receivingAddressInformation.consigneeArea &&
                this.receivingAddressInformation.consigneeArea.replace(
                  /,/g,
                  ""
                ); //收货地址
          ...
          if (res.code == 200) {
            if (res.data) {
              ...
              this.buyerTranManagerAddOrderObject.consignee =
                this.receivingAddressInformation.consignee; //收货人
              this.buyerTranManagerAddOrderObject.consigneeAddress =
                this.receivingAddressInformation.address; //详细地址
              this.buyerTranManagerAddOrderObject.consigneeArea =
                this.receivingAddressInformation.areacode; //收货地址
              this.buyerTranManagerAddOrderObject.contactPhone =
                this.receivingAddressInformation.phone; //联系电话
            } else {
              this.jumpTitle =
                "收货地址尚未设置,是否自动跳转到收货地址管理页面进行设置?";
              this.jumpValue = 2; //jumpValue 1跳转账号管理页面设置支付密码-2跳转收货地址管理页面 设置收货地址
    ...
    purchaseOnekeyBuyerDataProcessing() {
      // this.orderTotalSum ={
      //   amount:this.AllgenerateOrderData[0].amount,
      //   transportFee:this.AllgenerateOrderData[0].transportFee,
      //   payAmount:this.AllgenerateOrderData[0].payAmount,
      // }
      this.orderTotalSum.transportFee =
        this.AllgenerateOrderData[0].transportFee;
      this.computePrice();
      // this.AllgenerateOrderData = this.AllgenerateOrderData.slice(0, -1);
      this.buyerTranManagerAddOrderObject.supplierInfoList = [];
      console.log(this.AllgenerateOrderData);
      for (let i = 0; i < this.AllgenerateOrderData.length; i++) {
        this.AllgenerateOrderData[i].centerShopNowVo = [
          this.AllgenerateOrderData[i].centerShopNowVo,
        ];
        let orderJcGoodsList = [];
        this.AllgenerateOrderData[i].centerShopNowVo.forEach((element) => {
          let objects = {
            goodsId: element.goodsId, //  货品id
            goodsName: element.goodsName, //  货品名称
            price: element.buyPrice, //  单价
            unit: element.property, //  单位
    ...
    computePrice() {
      console.log(this.orderParams.count);
      let sum =
        (this.AllgenerateOrderData[0].amount * 100 * this.orderParams.count) /
        100;
      let transportFee = this.AllgenerateOrderData[0].transportFee;
      console.log(sum, transportFee);
      this.orderTotalSum.amount = sum; // 总金额
      if (sum >= this.AllgenerateOrderData[0].decreaseAmount) {
        // 达到免邮金额
        this.orderTotalSum.payAmount = sum;
      } else {
        this.orderTotalSum.payAmount = sum + transportFee;
      }
    },
    // 改变商品数量
    ...
    buyerTranManagerAddOrderApi() {
      console.log(this.buyerTranManagerAddOrderObject);
      if (
    ...
    generateOrderHandleOk() {
      if (this.$store.state.vuex_user.sysType == 5) {
        ...
        this.buyerTranManagerAddOrderApi(); // 采购商-新增集采订单-接口
      }
    },
    ...
    sysBuyerAddOrderApi() {
      ...
      this.loading = true;
      let orderJcGoodsList = [];
      this.AllgenerateOrderData[0].centerShopNowVo.forEach((element) => {
        let objects = {
      ...
      let valueData = {
        ...
        consignee: this.buyerTranManagerAddOrderObject.consignee, //收货人
        consigneeAddress: this.buyerTranManagerAddOrderObject.consigneeAddress, //详细收货地址
        consigneePhone: this.buyerTranManagerAddOrderObject.contactPhone, //收货人电话
        payAmount: this.orderTotalSum.payAmount, //支付金额
        payType: this.buyerTranManagerAddOrderObject.fkType, //支付方式
        supplierId: this.AllgenerateOrderData[0].supplierId, //供应商id
        transportFee: this.orderTotalSum.transportFee, //运费
        orderLsDetails: orderJcGoodsList,
      };
      sysBuyerAddOrder(valueData)
        .then((res) => {
          this.loading = false;
          if (res.code == 200) {
            this.payType = null;
