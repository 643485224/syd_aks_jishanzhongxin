<template>
    <div class="page-container table-page">
      ...
      <div class="header">
        <h2 class="header-title">物业批量新增</h2>
      </div>
      <div class="content">
        <div class="info">
          <div class="info-title">批量导入费用</div>
          <div class="info-content" :class="checkVisible?'noshow':''">
            <div class="info-img">
              <img v-lazy src="./image/download.png">
            </div>
            
            <div class="info-cont">
              <el-button type="text" class="downqx" @click="back">取消</el-button>
              <cu-upload-file ref="upload" :files.sync="excelData" :drag="false" accept=".xls,.xlsx,.csv" width="110px"
              ...
              <div class="check-list" v-if="wrong">
                <div class="check-item " v-for="(item,index) in checkList" :key="index">
                  <template v-if="!expand">
                    <div v-if="index < 7 || index > wrong - 3"> 【第{{item.rowNum}}行】{{item. errorText}}</div>
                    <div v-else-if="index == 7" class="tooltip" @click="expand=true">&emsp;...... <span
                        class="tooltiptext">点击展开</span></div>
                  </template>
                  <template v-else>
                    <div>【{{item.errorText}}】{{item.info}}</div>
                  </template>
                </div>
              </div>
  ...
  <script>
  import { downloadGoods } from "@/api/aksApi/platformApi/supplierPlatformApi.js";
  import { batchAdd, propertyFeeImportExcelCheck } from "@/api/aksApi/platformApi/operationPlatformApi.js";
  ...
  export default {
    ...
    methods: {
      ...
      async batchImport() {
        ...
        let [err, res] = await catchAwait(batchAdd(this.rightList))
        console.log(err,res);
        if (err) {
          console.log(err);
        }
      ...
      async customSubmit(file) {
        ...
        let func = this.ypqxType == 1?propertyFeeImportExcelCheck:propertyFeeImportExcelCheck;
        ...
        let { rightObjectList, wrongObjectList, rightNum, wrongNum } = res.data;
        this.checkList = wrongObjectList;
        this.rightList = rightObjectList;
        this.right = rightNum
        this.wrong = wrongNum
      },
      back() {
      ...
      exportXls() {
        ...
        if (res.code == 200) {
          ...
          console.log(res);
          this.downloads('https://ydwsrz-20220710.eos-xinjiang-1.cmecloud.cn/goods/%E7%89%A9%E4%B8%9A%E8%B4%B9%E7%94%A8%E5%AF%BC%E5%85%A5%E6%A8%A1%E6%9D%BF.xls');
        }
        else {
