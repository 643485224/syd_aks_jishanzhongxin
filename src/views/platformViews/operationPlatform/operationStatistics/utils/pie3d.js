...
export function getPie3D(pieData, internalDiameterRatio) {
    // myChart = document.getElementById(dom);
    let series = [];
    let sumValue = 0;
    let startValue = 0;
    let endValue = 0;
...
// 监听点击事件，实现选中效果（单选）
myChart&&myChart.on('click', function(params) {
...
myChart&&myChart.on('mouseover', function(params) {
...
// 修正取消高亮失败的 bug
myChart&&myChart.on('globalout', function() {
