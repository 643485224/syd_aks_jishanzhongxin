<template>
  <div class="page-container table-page" ref="contentBox">
    ...
    <cu-animation :type="'slideInRight'" :time=".5">
      <cu-table class="sale-table" :loading="tableLoading" :height="tableHeight" :showTitle="false"
        ...
        <template slot="roles" slot-scope="value">
          <template v-if="value.value.roles.length">
            <el-tag size="small" class="tag" v-for="(item,index) in value.value.roles" :key="index">{{item.roleName}}
            </el-tag>
          </template>
          <span v-else></span>
        </template>
    ...
    <cu-dialog :title="(formType === 'add' ? '新增' : '修改') + '账号'" width="50vw" :visible="formVisible" :showClose="true"
      ...
      <cu-form ref="form" :formData.sync="account" :formRule="formRule" class="form" @submitForm="submitForm"
        ...
        <template v-if="formType == 'edit'" v-slot:password="value">
          <el-input type="password" v-model="value.form[value.value.key]" :placeholder="value.value.placeholder"
            :disabled="value.value.disabled" autocomplete="new-password"></el-input>
...
import { userList, roleList, saveUser, updateUser, updateUserStatus, delUser, listExportXls } from "@/api/aksApi/platformApi/supplierPlatformApi.js";
