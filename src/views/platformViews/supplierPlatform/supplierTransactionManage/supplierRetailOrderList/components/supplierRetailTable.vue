<template>
  <div>
    ...
    <cu-table class="sale-table" :loading="tableLoading" :height="height" :showTitle="false" :tableHeader="tableHeader"
        ...
        <div class="expand-info">
          ...
          <div class="content">
            <div class="item">
              ...
              <div class="value">{{ value.value.contractNo }}</div>
            </div>
            <div class="item">
              <div class="label">商品种类:</div>
              <div class="value">{{ value.value.goodsCategoryCount }}</div>
            </div>
            <div class="item">
              <div class="label">支付方式:</div>
              <div class="value">{{ payType[value.value.fkType] }}</div>
            </div>
            <div class="item">
              <div class="label">交易时间:</div>
              <div class="value">{{ value.value.orderTime ? new Date(value.value.orderTime).Format("yyyy-MM-dd") : "" }}
              </div>
            </div>
      ...
      <template slot="operate" slot-scope="value">
        <el-button type="text" class="orange" @click="operation('returnPass', value.value.id)"
          v-if="value.value.status == 8">退货审核 </el-button>
        <el-button type="text" class="orange" @click="ship(value.value.orderNo,value.value.id)" v-if="value.value.status == 2">
          发货</el-button>
        <el-button type="text" class="orange" @click="operation('audit', value.value.id)"
          v-if="value.value.status == 9">取消审核
        </el-button>
    ...
    <cu-dialog :title="'发货'" width="50vw" :visible="shipVisible" :showClose="true" @handleClose="shipClose"
      ...
      <div class="dialog-form">
        <p>物流公司:</p>
        <el-input class="selectInput" v-model="expressCompany" placeholder="请填写物流公司">
        </el-input>
        <p>物流单号:</p>
...
import { getAllOrderLs, acceptOrder, rejectOrder, lsgoodsShipped, lsorderCancelAudit, lsorderReturnPass } from "@/api/aksApi/platformApi/supplierPlatformApi.js";
...
export default {
  ...
  data() {
    return {
      ...
      tableHeader: [
        ...
        {
          title: "订单状态",
          slot: "status",//  状态@ 1 待支付  2  待发货（已支付）  3  待收货    4  待评价（已收货）5 已完成   6   已取消   7  已退货  8  取消中  9  退货中
        },
        {
          title: "交易时间",
          slot: "payTime",
  ...
  methods: {
    ...
    showDetails(orderId, orderNo) {
      ...
      this.$router.push({ name: 'supplierRetailOrderDetails', params: { orderId, orderNo } })
    ...
    confirmShip() {
      if (this.expressCompany.trim() == '') {
        return this.$message.error('请输入物流公司')
      }
      if (this.expressNumber.trim() == '') {
        return this.$message.error('请输入物流单号')
      }
      lsgoodsShipped({ id: this.orderI, expressNo: this.expressNumber, expressCompany: this.expressCompany}).then(res => {
        console.log(res);
        if (res.code == 200) {
          this.clearDialog()
          this.shipVisible = false;
