        ...
        <div class="info-content ">
          <el-form-item class="info-item" label="药品标准:">
            ...
            <cu-upload-file type="file" ref="uploadStandrand" :files.sync="standrandFileList" text="添加附件" tip=""
              @submit="submitFile(arguments,'drugStandardsUrl')" :limit="1"
        ...
        <div class="info-content details-info">
          <el-form-item class="info-item" label="主图:">
            <cu-upload-file :files.sync="images" class="image-upload" text="添加主图" tip="" :limit="5" :maxSize="1"
              @submit="submitFile(arguments,'mainPicture')" @remove="removeFile(arguments,'mainPicture')" :multiple="true" accept=".png,.jpg,.jpeg">
            </cu-upload-file>
...
<script>
import { getBuyerGroupList, getAddDrugList, getMallClassificationList, addSupplierGoods, editEcho, edit } from "@/api/aksApi/platformApi/supplierPlatformApi.js";
    ...
    getEditEcho() {
      editEcho({ sgId: this.productId }).then(res => {
        ...
        if (res.code == 200) {
          ...
          // this.images = [{ name: '主图', url: data.mainPicture }]
          console.log(this.product);
        } else {
