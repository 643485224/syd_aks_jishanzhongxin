          ...
          <cu-table class="sale-table" :loading="tableLoading" :height="tableHeight" :showTitle="false"
              ...
              <template slot="roles" slot-scope="value">
                  <template v-if="value.value.roles.length">
                      <el-tag size="small" class="tag" v-for="(item,index) in value.value.roles" :key="index">{{item.roleName}}</el-tag>
                  </template>
                  <span v-else ></span>
...
<style lang="scss" src="./authorityAccountManagement.scss" scoped>
</style>
