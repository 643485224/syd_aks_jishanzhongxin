<template>
  <div class="page-container table-page" ref="contentBox">
    ...
    <cu-animation :type="'slideInRight'" :time="0.5">
      ...
      >
        ...
        <template slot="roles" slot-scope="value">
          <template v-if="value.value.roles ? value.value.roles.length : null">
            <el-tag
              size="small"
              class="tag"
              v-for="(item, index) in value.value.roles"
              :key="index"
          ...
          <!-- <template v-if="value.value.roles.length > 1">
                    ...
                    <template v-else>
                        <el-tag  v-if="value.value.roles.length" size="small">{{value.value.roles[0].roleName}}</el-tag>
                        <span v-else ></span>
                    </template> -->
        </template>
        ...
        <template slot="operate" slot-scope="value">
          <el-button
            type="text"
            class="warning-btn"
            v-if="value.value.status == 1"
            @click="updateStatus(value.value.id, 0)"
            >禁用</el-button
          >
          <el-button
            type="text"
            ...
            @click="updateStatus(value.value.id, 1)"
            >启用</el-button
          >
          <el-button type="text" @click="showForm('edit', value.value.id)"
            >修改</el-button
          >
          <el-button
            type="text"
            class="warning-btn"
            @click="deleteUser(value.value.id, 1)"
            >删除</el-button
          >
        </template>
      </cu-table>
    ...
    >
      ...
      <cu-form
        ref="form"
        :formData.sync="account"
        :formRule="formRule"
        class="form"
        @submitForm="submitForm"
        @closeForm="closeForm"
        @rolesGetData="rolesGetData"
        @rolesInitData="rolesInitData"
        ...
        if (type == "edit") {
          ...
          this.account = {
            ...
            roles: editData.roles
              ? editData.roles.map((item) => item.id)
              : null,
...
<style lang="scss" src="./authorityAccountManagement.scss" scoped></style>
