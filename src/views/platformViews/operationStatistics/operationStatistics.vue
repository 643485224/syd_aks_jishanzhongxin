      ...
      <div class="nav">
        <div class="nav-left">
          <div class="nav-title-wrapper">
            <div class="nav-title">阿克苏药品集散中心</div>
          </div>
        </div>
        <div class="nav-right">
              ...
              <div class="sales-list">
                <div class="sales-item" v-for="(item, index) in salesData" :key="index">
                  <div class="sales-text">
                    <div class="text-comp"
                      :style="{ color: index> (colors.length - 1)?colors[index%colors.length]:colors[index] }"><span
                        class="legend-sign" :style="{ 'border-color': colors[index] }"></span>{{
                        item.unitName }}</div>
                    <div class="text-count">{{ item.orderCount }}</div>
                  </div>
                  <el-progress :text-inside="false" :show-text="false" :stroke-width="18"
                    v-if="!isNaN(parseInt(item.rate*100))" :color="colors[index]" :percentage="item.rate * 100">
                  </el-progress>
                </div>
              </div>
            </div>
          ...
          <div class="increase-trend card center-card">
            ...
            <div class="card-content">
              <div class="trend-left">
                <div class="top">
                  <div class="percentage"><span class="num">{{ yearTradeRiseData.jyeSsbl }}</span>%
                  </div>
                ...
                <div class="bottom">
                  <div class="percentage"><span class="num">{{ yearTradeRiseData.qysSsbl }}</span> %
                  </div>
