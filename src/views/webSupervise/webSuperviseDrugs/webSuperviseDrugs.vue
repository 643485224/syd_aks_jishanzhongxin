<template>
  <div class="equipment">
    ...
    <navigation index="1" class="top70"></navigation>
    <div class="equipment-item">
      <div class="item">
        ...
        >
          <el-option
            v-for="item in superviseDrugManageSelectReturn"
            :key="item.index"
            :label="item.name"
            :value="item.id"
          ></el-option>
    ...
    >
      <ul>
        <li
          v-for="(item, index) in superviseDrugManageSelectReturn"
          :key="index"
    ...
    <RegulatorySearch
      ...
      v-show="ifTrueFalse"
      :spType="1"
      :list="superviseDrugManageSelectDrugListReturn"
...
<script>
import {
  superviseDrugManageSelect,
  superviseDrugManageSelectDrugList,
...
export default {
  ...
  data() {
    return {
      ...
      ifTrueFalse: false,
      elInputValue: "",
      superviseDrugManageSelectReturn: [], // 获取药品分类-接口回参
      superviseDrugManageSelectDrugListReturn: [], // 获取药品列表-接口回参
      currPageNo: 1,
      pageSize: 10,
      total: 0,
      typeLoading: false,
  ...
  mounted() {
    this.$nextTick(() => {
      this.superviseDrugManageSelectApi(); // 获取药品分类-接口
    });
  },
  methods: {
    // 获取药品分类-接口
    superviseDrugManageSelectApi() {
      ...
      superviseDrugManageSelect()
        .then((res) => {
          ...
          if (res.code == 200) {
            ...
            this.superviseDrugManageSelectReturn = res.data; // 获取药品分类-接口回参
            let object = {
              id: "-1",
              name: "全部",
            };
            this.superviseDrugManageSelectReturn.unshift(object);
            if (
              ...
              this.superviseDrugManageSelectDrugListApi(); // 获取药品列表-接口
            }
          } else {
    ...
    // 获取药品列表-接口
    superviseDrugManageSelectDrugListApi() {
      ...
      superviseDrugManageSelectDrugList(valueData)
        .then((res) => {
          this.listLoading = false;
          if (res.code == 200) {
            this.superviseDrugManageSelectDrugListReturn = res.data.records; // 获取药品列表-接口回参
    ...
    handleSizeChange(val) {
      this.currPageNo = 1;
      this.pageSize = val;
      this.superviseDrugManageSelectDrugListApi();
    },
    // 页码改变回调
    ...
    selectClick(value) {
      if (value) {
        ...
        this.superviseDrugManageSelectDrugListReturn = [];
        this.ifTrueFalse = true;
        this.superviseDrugManageSelectDrugListApi(); // 获取药品列表-接口
      } else {
